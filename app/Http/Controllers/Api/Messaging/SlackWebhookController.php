...
class SlackWebhookController extends Controller
{
    ...
    {
        // Nothing writes to the database before this point. A debug insert
        // used to run first, unconditionally, against a hardcoded
        // message_channel_id - which both let any unauthenticated caller
        // create conversation rows and 500'd the whole endpoint with an FK
        // violation whenever that literal id didn't exist locally. Slack
        // retries a failing Request URL and eventually disables it, so that
        // crash took down every delivery, images included.
