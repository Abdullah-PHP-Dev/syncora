                    ...
                    <tbody id="apiTable" class="table-border-bottom-0">
                        @foreach ($campaigns as $campaign)
                            <tr data-key="{{ $campaign->key }}">
                                ...
                                <td>{{ $campaign->end_time ? \Carbon\Carbon::parse($campaign->end_time)->format('d M Y, h:i A') : '-' }}</td>
                                <td>
                                    <div class="d-flex align-items-center">
                                        <div class="dropdown">
                                            ...
                                            <div class="dropdown-menu dropdown-menu-end">
                                                <a href="{{ route('admin.ads.campaigns.edit', ['platform' => 'linkedin', 'campaign' => $campaign->id]) }}" data-key="{{ $campaign->key }}"
...
@push('scripts')
    <script>
        ...
        var destroyAPIUrl = "{{ route('admin.ads.campaigns.destroy', ['platform' => 'linkedin', 'campaign' => ':API']) }}";
        var statusAPIUrl = "{{ route('admin.ads.campaigns.status', ['platform' => 'linkedin', 'id' => ':API']) }}";
