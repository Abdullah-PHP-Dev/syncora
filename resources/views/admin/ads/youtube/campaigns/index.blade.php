                    ...
                    <tbody id="apiTable" class="table-border-bottom-0">
                        @foreach ($campaigns as $campaign)
                            <tr data-key="{{ $campaign->key }}">
                                <td>{{ $campaign->id }}</td>
                                <td>{{ $campaign->name }}</td>
                                <td>{{ Str::title(str_replace('_', ' ', $campaign->bidding_strategy)) }}</td>
...
@push('scripts')
    <script>
        ...
        var destroyAPIUrl = "{{ route('admin.ads.campaigns.destroy', ['platform' => 'youtube', 'campaign' => ':API']) }}";
        var statusAPIUrl = "{{ route('admin.ads.campaigns.status', ['platform' => 'youtube', 'id' => ':API']) }}";
